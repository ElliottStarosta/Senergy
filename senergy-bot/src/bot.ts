    ...
    .addSubcommand((sub: any) =>
      sub
        ...
        .addIntegerOption((opt: any) =>
          opt
            .setName('radius')
            .setDescription('Search radius in km (5-50, default: 15)')
            .setRequired(false)
            .setMinValue(5)
            .setMaxValue(50)
        )
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isButton()) {
    try {
      ...
      }else if (customId === 'group_rate_place') {
        // Handle "Rate This Place" button
        const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173'
        await interaction.reply({
          content: `⭐ Rate this place at ${frontendUrl}/rate`,
          ephemeral: true
        })
